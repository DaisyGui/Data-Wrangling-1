...
**janitor::clean_names** use clean_names function in janitor package. No
need to library(janitor). Use when this is the only function used from
janitor in this code.
...
*col_names = FALSE* indicated there is no column names row, this way the
Xn are used as default column names instead of reading the first row as
column names.
...
*na = c()* detect content in the list and take them as NA when reading
csv
...
*sheet* when there is multiple sheets in excel file *range* to specify
row or column range to read in
